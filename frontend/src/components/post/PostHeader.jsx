...
export default function PostHeader(props) {
  ...
  <div>
    <Link to={`/user/${props.userID}`}><img className="object-contain w-10 h-10 rounded-full shadow" src={imagePath} /></Link>
  </div>

  <div className="flex items-center">

    <Link to={`/user/${props.userID}`}><h2 className="text-lg mx-2 font-semibold text-gray-900">{`${props.firstName}` + ` ` + `${props.lastName}`}</h2></Link>
